...
const CurrentTeamsAndServices = ({values, setFieldValue, SwitchTab, organization}) => {
  const {data: session} = useSession();
  const {t} = useTranslation();

  const handleShowManageService = () => {
  ...
  return (
    <div className='flex flex-col w-full h-full md:flex-row duration divide-y md:divide-y-0 md:divide-x '>
      <div className='w-full md:w-4/12 h-full md:h-max'>
      ...
      <div className='w-full h-max md:w-8/12 md:ml-2'>
        <div className='flex p-2 justify-end items-center'>
          <Button variant='solid' className='w-full sm:w-max' onClick={handleShowManageService}>
