...
import {PencilAltIcon, TrashIcon, EyeIcon, EyeOffIcon} from '@heroicons/react/outline';
...
export default function Actions({button, serviceID, refetch, slug, isPrivate}) {
        ...
        >
          <Menu.Items className='absolute flex flex-col right-11 xl:right-8 -bottom-0 xl:-bottom-14 w-40 origin-top-right rounded-md bg-white shadow-lg border z-50 p-4'>
            <Menu.Item>
              ...
              >
                {!isPrivate ? (
                  <EyeOffIcon className='w-5 h-5 mr-2 text-black' />
                ) : (
            ...
            <Menu.Item>
              <button
                onClick={() =>
                  router.push(`/user/services/service-form/?mode=edit&sid=${serviceID}`)
                }
                className='flex px-3 py-2 hover:bg-gray-200 hover:bg-opacity-50 rounded-md'
              >
                <PencilAltIcon className='w-5 h-5 mr-2 text-mainBlue' />
                {t('btn_edit')}
              </button>
            </Menu.Item>
            <Menu.Item>
