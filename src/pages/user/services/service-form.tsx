...
const AddService = () => {
  ...
  const {mode, sid, stype} = router.query;
    ...
    try {
      ...
      if (action === 'back' && step > 1) {
        if (
          ...
          (values.servicePaid === 'no' || values.servicePayMethod === 'manual')
        ) {
          setStepName(
  ...
  return (
    <PostLoginLayout className='pb-28 mb-10'>
      ...
      <Header
        crumbs={crumbs}
        heading={`${mode === 'create' ? t('add_new_service') : t('edit_service')} ${serviceType}`}
      />
      {moreOptions && (
        <MoreOptions
      ...
      {moreOptions && (
        <EditBar
          disabled={
            values.servicePaid === 'yes' && values.servicePayMethod !== 'manual' ? [] : ['Charity']
          }
          ...
          <>
            ...
            <Transition step={step} show={created}>
              <div className='flex flex-col items-center justify-center'>
                <BadgeCheckIcon className='w-12 h-12 text-lime-600' />
