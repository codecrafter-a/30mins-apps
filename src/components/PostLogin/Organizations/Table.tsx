...
const Table = ({orgs, userId, refetch}) => {
  ...
  const handleDeleteOrganization = useCallback(
    ...
    [deleteMutation, router, session?.accessToken, showNotification]
  );
  ...
  const handleLeaveOrganization = useCallback(
    async organization => {
      try {
        ...
        if (data?.leaveOrganization?.status === 400) {
          showNotification(NOTIFICATION_TYPES.info, data?.leaveOrganization?.message, false);
        } else {
          refetch();
          hideModal();
        }
    ...
    [leaveMutation, router, session?.accessToken, showNotification]
  );
                ...
                return (
                  <tr key={orgID} className='bg-white border-b w-full'>
                    ...
                    <td colSpan={1} className='p-2'>
                      <span className='font-semibold w-32 md:w-3/4 mx-auto break-words text-left line-clamp-2'>
                        {organization?.title}
                      </span>
                    </td>
                    <td colSpan={1} className='p-2'>
