...
const GeneralEdit = ({userData}) => {
  ...
  const handleFullName = async ({target: {value}}) => {
    ...
    if (!validate(value)) {
      setFullnameError(t('profile:one_space_allowed'));
      return;
    }
    ...
    if (value === '' || value === ' ') {
      ...
      try {
        await updateUser({
          variables: {
            userData: {personalDetails: {name: value}},
            token: session?.accessToken,
          },
        });
      } catch (e) {
  ...
  const handleMediaLink = async ({target: {value}}) => {
    ...
    if (profileMediaType === 'Google Slides') {
      val.startsWith('https://docs.google.com/presentation/')
        ? await updateUser({
            variables: {
              userData: {
        ...
        : setMediaLinkError('Expected Google Link: https://docs.google.com/presentation/30mins');
      return;
    }
