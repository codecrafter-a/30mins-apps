...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!status || !authCode) {
      res.redirect(`/Home?message=Some params missing`);
      return;
    }

    const [statusUser, actualStatus] = status.split('-');
    ...
    if (statusAuthCodeResult.data.getStatusAuthCode.response.status !== 200) {
      res.redirect(`/Home?message=${statusAuthCodeResult.data.getStatusAuthCode.response.message}`);
      return;
    }

    if (statusAuthCodeData.used) {
      res.redirect(`/Home?message=Status Auth Code has been used already`);
      return;
    }

    if (dayjs().isBefore(statusAuthCodeData.expireAt)) {
      if (postBookingStatus.includes(status)) {
        res.redirect(`/Home?message=cannot ${actualStatus} before booking time`);
        return;
      }
    }
    ...
    if (dayjs().isAfter(statusAuthCodeData.expireAt)) {
      if (preBookingStatus.includes(status)) {
        res.redirect(
          `/Home?message=You can not ${actualStatus} any meeting which has already past away their time slot.`
        );
        return;
      }
    }
    ...
    if (
      ...
      bookingStatus.clientCanceled ||
      bookingStatus.providerCanceled ||
      bookingStatus.providerDeclined ||
      bookingStatus.hasOpenReport
    ) {
      res.redirect(`/Home?message=Cannot Process Your Request`);
      return;
    }

    const toSendEmail =
      statusUser === 'provider' ? bookingData.bookerEmail : bookingData.providerEmail;
    ...
    if (status === 'provider-reply') {
      ...
      res.redirect(`/Home?message=E-mail has been sent.`);
      return;
    }
    ...
    if (statusAuthCodeData.availableStatus.includes(status)) {
      ...
      res.redirect(`/Home?message=Status Auth Code Invalid`);
      return;
    }
    ...
    if (updateBookingResult.data.updateBookingStatus.status !== 200) {
      res.redirect(`/Home?message=Could not update the booking status`);
      return;
    }
    ...
    res.redirect(`/Home?message=Meeting ${actualStatus} Successful`);
    return;
  } catch (error) {
    res.redirect(`/Home?message=Something went wrong`);
  }
}
