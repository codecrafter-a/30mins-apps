    ...
    >
      {loading ? (
        ...
        roundRobinTeams?.map((item, index) => (
          <li key={index} className='grid grid-cols-3 grid-flow-row p-2 shadow-sm border-2 rounded'>
            <div className='grid col-span-2 content-around'>
                  ...
                  <Menu.Items className='origin-top-right absolute right-4 -top-1 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none'>
                    <div className='py-1'>
                      ...
                      <Menu.Item>
                        {({active}) => (
                          <button
                            onClick={() => deleteTeam(item._id, item.name)}
                            className={classNames(
                              active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                              'w-24 px-4 py-2 text-sm  flex'
                            )}
