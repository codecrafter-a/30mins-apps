...
export default function StepOne({
  ...
  const onUpload = async () => {
    try {
      ...
      // );
      const Data = await Jimp.read(base64).then(lenna => lenna.resize(size.w, size.h));
      const data: any = await Data?.getBufferAsync(Jimp.MIME_PNG);
          ...
          <div className='image-editor__cropper flex justify-center items-center '>
            ...
            {Boolean(imgSrc) === false && (
              <div className='w-full h-full flex flex-col justify-center items-center'>
                ...
                <PhotographIcon className='w-28 text-white opacity-25' />
              </div>
            )}
            {mode !== 'crop' && (
              <Slider
