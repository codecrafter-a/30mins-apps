...
export default function Stepper({
  ...
  'MEETING' | 'FULL_TIME_JOB' | 'PART_TIME_JOB' | 'FREELANCING_WORK' | 'ROUND_ROBIN' | ''
>) {
  ...
  useEffect(() => {
    if (modalType === 'send_otp')
      showModal(MODAL_TYPES.MESSAGE_OTP_MODAL, {
        values,
        setFieldValue,
        showMeetingOTP,
        signInSubmitHandler,
  ...
  useEffect(() => {
    if (values.verify_otp && values.step === 0) {
      showModal(MODAL_TYPES.MESSAGE_OTP_VERIFY, {
        showMeetingOTP,
        bookerEmail: values?.bookingData?.bookerEmail,
        move,
      });
        ...
        {values.STEPS[values.step] !== 'payment' && (
          <div className='grid grid-cols-2 gap-2 w-full md:w-1/3 justify-evenly items-end'>
            ...
            <Button
              ...
              className='col-span-1'
              onClick={async () => {
                if (values?.checked && !values?.verify_otp) {
                  console.log('Hello SEnd otp');
                  setModalType('send_otp');
                  setFieldValue('selectOtp', true);
                  // return null;
                } else if (values.bookingData.providerEmail !== values.bookingData.bookerEmail) {
                  if (loading === false) {
                    setLoading(true);
